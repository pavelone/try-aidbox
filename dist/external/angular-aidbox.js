...
/***/ function(module, exports, __webpack_require__) {
	...
	var URI, addMinutes, addSeconds, mod;

	URI = __webpack_require__(2);
	...
	mod.service('$aidbox', function($http, $cookies, $window, $q) {
	  var access_token, box_url, config, flow, http, loginUrl, out, query, user_state;
	  config = {
	    client_id: 'site',
	    grant_type: 'implicit',
	    scope: 'ups',
	  ...
	  user_state = function(state, user) {
	    if (config.onUser) {
	    ...
	    switch (state) {
	      ...
	      case 'signout':
	        ...
	        break;
	      case 'anonimous':
	        if (config.onAnonimous) {
	          return config.onAnonimous();
	        }
	    }
	  };
	  out = function() {
	  ...
	  http = function(opts) {
	    ...
	    return $http(args).error(function(data, st) {
	      if (st === 403) {
	        return out();
	      }
	    });
